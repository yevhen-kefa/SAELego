...
class OrderController {
    ...
    public function process() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $sqlMosaic = "INSERT INTO mosaic (uploads_id, filter_used, size_option, estimated_price, created_at) 
                          VALUES (:uid, :filter, :size, :price, NOW())";
            $stmt = $pdo->prepare($sqlMosaic);
            $stmt->execute([
                ':uid' => $_POST['upload_id'],
                ':filter' => $_POST['filter_css'],
            ...
            $invoiceData = [
                ...
                'items' => [
                    [
                        'description' => "Mosaïque LEGO® personnalisé (" . $_POST['size_option'] . "x" . $_POST['size_option'] . ")",
                        'filter'      => $_POST['filter_css'],
                        'quantity'    => 1,
            ...
            $jsonContent = json_encode($invoiceData);
